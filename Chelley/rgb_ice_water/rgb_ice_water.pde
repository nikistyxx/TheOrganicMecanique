...
void draw() {
  for (int x = 1; x < width-1; x++) {
    for (int y = 1; y < height-1; y++) {
      v1[x][y] += (z1[x-1][y] + z1[x+1][y] + z1[x][y-1] + z1[x][y+1])*8 - z1[x][y];
      v2[x][y] += (z2[x-1][y] + z2[x+1][y] + z2[x][y-1] + z2[x][y+1] * 8 + z1[x][y])  - z2[x][y];
    }
  }
  ...
  for (int x = 1; x < width-1; x++) {
    for (int y = 1; y < height-1; y++) {
     ...
     z2[x][y] = constrain(z2[x][y], -1, 1);
      pixels[width*y+x] = color(v1[x][y] + 199, v2[x][y] + 30, 255); //control color value
    }
  }
